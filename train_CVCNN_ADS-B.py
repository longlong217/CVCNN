...
from CVCNN_model import *#导入了名为model_complexcnn_onlycnn的模块
from get_dataset_ADSB import TrainDataset, TestDataset #导入了名为 TrainDataset 和 TestDataset 的类，它们来自 get2 模块
import random
import os
...
class Config:
    def __init__(
            ...
            epochs: int = 300,#训练轮数
            lr: float = 0.001,
            n_classes: int = 20,#涉及的分类数
            save_path: str = 'model_weight/train_ADS-B_CVCNN.pth',
...
def main():
    conf = Config()
    writer = SummaryWriter("logs_train_ADS-B_CVCNN_rand" + str(conf.rand_num))#这个路径将用于存储TensorBoard日志文件
